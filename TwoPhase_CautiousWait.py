...
#         print("Transaction "+TID+ " goes into blocked state (state = blocked)")
...
#method to execute cautious Wait mechanism
def cautiousWait(TID, lockList, dataItem, row):
    global transactionTable
    global lockingTable
    global blockTable

    tidList = []
    isAtleastOneBlocked = False

    for ele in lockList['transactionIDHolding'].to_list():
        if(ele != TID):
            status = getTransactionStatus(ele)
            tidList.append(ele)
            if status == 'Blocked':
                isAtleastOneBlocked = True
  

    if isAtleastOneBlocked:
        print("Transaction "+TID+" is aborted (state = Aborted)")
        abort(TID)
...
def readLock(row, TID):
    ...
    if lockList.size == 0:
        ...
        if atleastOneWrite:
            cautiousWait(TID, lockList, dataItem, row)
        else:
            print("Transaction "+TID+ " acquired a read lock on data item "+dataItem)
            lockingTable = lockingTable.append(pd.Series([dataItem, 'R', TID], index=lockingTable.columns ), ignore_index=True)
...
def writeLock(row, TID):
    ...
    if len(lockList) == 0:
        ...
        # if olderTID:
        #     print("Transaction "+TID+ " changed to blocked status")
        #     transactionTable.loc[transactionTable['transactionID'] == TID, ['transactionStatus']] = 'Blocked'
        ...
        # for tid in youngerTID:
        #     print("Transaction "+tid+ " is aborted")
        #     abort(tid, 'Aborted')
